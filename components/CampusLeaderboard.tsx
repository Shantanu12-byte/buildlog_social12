...
import { Colors as ThemeColors, Typography, Spacing, Radius } from '@/constants/theme';
...
function TopThreeCard({ user, rank, onPress }: { user: LeaderboardUser; rank: number; onPress: () => void }) {
  ...
  const glowColor = isGold ? COLORS.gold : isSilver ? COLORS.silver : COLORS.bronze;
  ...
  return (
    ...
    >
      ...
      <View style={s.rowStreak}>
        <MaterialCommunityIcons name="fire" size={16} color={COLORS.streak} />
...
export default function CampusLeaderboard() {
  ...
  const [, setTick] = useState(0); // For forcing UI update every minute
